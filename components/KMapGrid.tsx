...
import type { KMapValue, KMapConfig } from '../types';
...
const KMapGrid: React.FC<KMapGridProps> = ({ kmapData, onMapChange, config }) => {
    const { rows, cols, rowVar, colVar, indices } = config;
    ...
    return (
        <div className="relative p-4 sm:p-6 select-none">
            <div className="grid" style={{ gridTemplateColumns: `auto repeat(${cols.length}, minmax(0, 1fr))` }}>
                ...
                {/* Column Headers */}
                {cols.map((col) => (
                    <div key={col} className="flex items-center justify-center h-10 sm:h-12 text-sm sm:text-base font-bold text-slate-300">
                        {col}
                    </div>
                ))}
                {/* Rows */}
                {rows.map((row, rowIndex) => (
                    <React.Fragment key={row}>
                        {/* Row Header */}
                        <div className="flex items-center justify-center w-10 sm:w-12 text-sm sm:text-base font-bold text-slate-300">
                            {row}
                        </div>
                        {/* Cells */}
                        {cols.map((_, colIndex) => {
                            const dataIndex = indices[rowIndex][colIndex];
                            return (
                                <button
                                    ...
                                    className={`flex items-center justify-center h-14 w-14 sm:h-16 sm:w-16 m-1 rounded-lg text-2xl sm:text-3xl font-mono font-bold transition-all duration-200 transform hover:scale-110 hover:z-10 shadow-md border border-slate-600/50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-fuchsia-500 ${getCellClass(kmapData[dataIndex])}`}
                                >
                                    {kmapData[dataIndex]}
                                </button>
                            );
